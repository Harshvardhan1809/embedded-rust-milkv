...
use crate::one_wire_bus::crc;
use crate::one_wire_bus::error::{OneWireError, OneWireResult};
...
impl OneWire {
    pub fn new(pin: CdevPin) -> OneWireResult<OneWire> {
        let mut one_wire = OneWire { pin };
        // Pin should be high during idle.
        // one_wire.release_bus()?;
        one_wire.pin.set_high().map_err(|err| OneWireError::PinError(Box::new(err)))?;
        Ok(one_wire)
    }

    pub fn into_inner(self) -> CdevPin {
        self.pin
    }

    /// Disconnects the bus, letting another device (or the pull-up resistor) set the bus value
    pub fn release_bus(&mut self) -> OneWireResult<()> {
        
        // self.pin = self.pin.into_input_pin().map_err(|err| OneWireError::PinError(err))


        // let line = self.pin.0.line().clone();
        // if line.info().map_err(|err| OneWireError::PinError(Box::new(err)))?.direction() == LineDirection::In {
        //     return Ok(());
        // }
        // println!("Here");
        // // TODO: ensure memory get deallocated properly std::mem::drop(self.pin);
        // std::mem::drop(self.pin);
        // let line_handle = line.request(LineRequestFlags::INPUT, 0, "one-wire-ds18b20").map_err(|err| OneWireError::PinError(Box::new(err)))?;
        // println!("There");
        // self.pin = CdevPin::new(line_handle).map_err(|err| OneWireError::PinError(Box::new(err)))?;
        // Ok(())

        self.pin.set_high().map_err(|err| OneWireError::PinError(Box::new(err)))
    }

    /// Drives the bus low; set as output
    pub fn set_bus_low(&mut self) -> OneWireResult<()> {
        // Idea - implement the function here entirely: 
        // let line = self.pin.0.line().clone();
        // if line.info().map_err(|err| OneWireError::PinError(Box::new(err)))?.direction() == LineDirection::Out {
        //     return Ok(());
        // }
        // // TODO: ensure memory get deallocated properly std::mem::drop(self.pin);

        // let line_handle = line.request(LineRequestFlags::OUTPUT, 1, "one-wire-ds18b20").map_err(|err| OneWireError::PinError(Box::new(err)))?;
        // self.pin = CdevPin::new(line_handle).map_err(|err| OneWireError::PinError(Box::new(err)))?;
        // Ok(())
        // Want to attempt : 
        // self.pin = self.into_output_pin(embedded_hal::digital::PinState::Low).map_err(|err| OneWireError::PinError(Box::new(err)))?;
        // Ok()

        // Original : 
        self.pin.set_low().map_err(|err| OneWireError::PinError(Box::new(err)))
    }

    pub fn is_bus_high(&mut self) -> OneWireResult<bool> {
        self.pin.is_high().map_err(|err| OneWireError::PinError(Box::new(err)))
    }

    pub fn is_bus_low(&mut self) -> OneWireResult<bool> {
        self.pin.is_low().map_err(|err| OneWireError::PinError(Box::new(err)))
    }

    fn wait_for_high(&mut self, delay: &mut impl DelayNs) -> OneWireResult<()> {
    ...
    pub fn reset(&mut self, delay: &mut impl DelayNs) -> OneWireResult<bool> {
        // delay.delay_ms(2000);
        println!("In delay 1");
        self.wait_for_high(delay)?;
        println!("In delay 2");
        println!("Bus is low : {}", !self.is_bus_high()?);
        self.set_bus_low()?;
        println!("Bus is low : {}", !self.is_bus_high()?);
        delay.delay_us(480); // Maxim recommended wait time
        println!("In delay 3");
        self.release_bus()?;
        delay.delay_us(1); // Maxim recommended wait time default was 70
        let device_present = self.is_bus_low()?;
        println!("Device present {:?}", device_present);
        delay.delay_us(410); // Maxim recommended wait time
        Ok(device_present)
    }

    pub fn read_bit(&mut self, delay: &mut impl DelayNs) -> OneWireResult<bool> {
        self.set_bus_low()?;
    ...
    pub fn read_byte(&mut self, delay: &mut impl DelayNs) -> OneWireResult<u8> {
        let mut output: u8 = 0;
        for _ in 0..8 {
            output >>= 1;
    ...
    pub fn write_1_bit(&mut self, delay: &mut impl DelayNs) -> OneWireResult<()> {
        self.set_bus_low()?;
        delay.delay_us(6); // Maxim recommended wait time
    ...
    pub fn write_0_bit(&mut self, delay: &mut impl DelayNs) -> OneWireResult<()> {
        self.set_bus_low()?;
    ...
    /// This should only be called after a reset, and should be immediately followed by another command
    pub fn skip_address(&mut self, delay: &mut impl DelayNs) -> OneWireResult<()> {
    ...
    pub fn device_search(
        ...
        only_alarming: bool,
        delay: &mut impl DelayNs,
    ) -> OneWireResult<Option<(Address, SearchState)>> {
        println!("1");
...
pub struct DeviceSearch<'a, 'b, D> {
    onewire: &'a mut OneWire,
    delay: &'b mut D,
...
impl<'a, 'b, D> Iterator for DeviceSearch<'a, 'b, D>
where
    D: DelayNs,
{
    type Item = OneWireResult<Address>;
